...
PYBIND11_MODULE(__pycaliper_impl, m) {
  ...
  m.def("is_initialized", &pycaliper_is_initialized,
        "Check if Caliper is initialized on this process.");

  auto types_mod = m.def_submodule(
      "types", "Special types used by lower-level Caliper APIs.");
  ...
  auto instrumentation_mod =
      m.def_submodule("instrumentation",
                      "Support for higher-level Caliper instrumentation APIs");
  ...
  auto config_mgr_mod = m.def_submodule(
      "config_manager", "Support for dynamic configuration of Caliper.");

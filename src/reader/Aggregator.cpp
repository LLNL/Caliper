...
std::vector<Entry> make_key(std::vector<const Node*>::const_iterator nodes_begin,
...
public:
    ...
    virtual bool is_inclusive() const {
        return false;
    }
...
public:
    ...
    const AggregateKernelConfig* config() {
        return m_config;
    }
...
public:
    ...
    public:
        ...
        Attribute get_count_attr(CaliperMetadataAccessInterface& db) {
            if (!m_count_attr) {
                m_count_attr = db.create_attribute(std::string("scount#") + m_scale_str,
                                                   CALI_TYPE_UINT,
...
public:
    ...
    public:
        ...
        Attribute get_sum_attr(CaliperMetadataAccessInterface& db) {
            if (!m_sum_attr)
                m_sum_attr =
                    db.create_attribute(std::string(m_is_inclusive ? "inclusive#" : "sum#") + m_target_attr_name,
                                        m_target_attr.type(),
...
public:
    ...
    public:
        ...
        Attribute get_sum_attr(CaliperMetadataAccessInterface& db) {
            if (!m_sum_attr)
                m_sum_attr = db.create_attribute(std::string(m_inclusive ? "iscsum#" : "scsum#") + m_target_attr_name,
                                                 CALI_TYPE_DOUBLE,
                                                 CALI_ATTR_ASVALUE | CALI_ATTR_HIDDEN);
            return m_sum_attr;
        }
        ...
        Attribute get_result_attr(CaliperMetadataAccessInterface& db) {
            if (!m_res_attr)
                m_res_attr = db.create_attribute(std::string(m_inclusive ? "iscale#" : "scale#") + m_target_attr_name,
                                                 CALI_TYPE_DOUBLE,
                                                 CALI_ATTR_ASVALUE);
            return m_res_attr;
        }
        ...
        Attribute get_min_attr(CaliperMetadataAccessInterface& db) {
            if (!m_min_attr) {
                if (m_target_attr) {
                    ...
                    m_min_attr = db.create_attribute(std::string(m_inclusive ? "imin#" : "min#") + m_target_attr_name,
                                                     type,
                                                     prop);
                }
            }
        ...
        Attribute get_max_attr(CaliperMetadataAccessInterface& db) {
            if (!m_max_attr) {
                if (m_target_attr) {
                    ...
                    m_max_attr = db.create_attribute(std::string(m_inclusive ? "imax#" : "max#") + m_target_attr_name,
                                                     type,
                                                     prop);
                }
            }
...
public:
    ...
    public:
        ...
        std::pair<Attribute, Attribute> get_sum_attributes(CaliperMetadataAccessInterface& db) {
            if (!m_sum1_attr)
                m_sum1_attr = db.create_attribute(std::string(m_inclusive ? "isr.sum#" : "sr.sum#") + m_tgt1_attr_name,
                                                  CALI_TYPE_DOUBLE,
                                                  CALI_ATTR_SKIP_EVENTS | CALI_ATTR_ASVALUE | CALI_ATTR_HIDDEN);
            if (!m_sum2_attr)
                m_sum2_attr = db.create_attribute(std::string(m_inclusive ? "isr.sum#" : "sr.sum#") + m_tgt2_attr_name,
                                                  CALI_TYPE_DOUBLE,
                                                  CALI_ATTR_SKIP_EVENTS | CALI_ATTR_ASVALUE | CALI_ATTR_HIDDEN);
        ...
        Attribute get_ratio_attribute(CaliperMetadataAccessInterface& db) {
            if (!m_ratio_attr)
                m_ratio_attr = db.create_attribute(std::string(m_inclusive ? "iratio#" : "ratio#") + m_tgt1_attr_name
                                                       + "/" + m_tgt2_attr_name,
                                                   CALI_TYPE_DOUBLE,
                                                   CALI_ATTR_SKIP_EVENTS | CALI_ATTR_ASVALUE);
            return m_ratio_attr;
...
public:
    ...
    public:
        ...
        bool get_percentage_attribute(CaliperMetadataAccessInterface& db,
            ...
            m_percentage_attr = db.create_attribute(std::string(m_is_inclusive ? "ipercent_total#" : "percent_total#")
                                                        + m_target_attr_name,
                                                    CALI_TYPE_DOUBLE,
                                                    CALI_ATTR_SKIP_EVENTS | CALI_ATTR_ASVALUE);
...
public:
    ...
    public:
        ...
        Attribute get_any_attr(CaliperMetadataAccessInterface& db) {
            if (!m_any_attr) {
                if (m_target_attr)
                    m_any_attr = db.create_attribute(std::string("any#") + m_target_attr_name,
                                                     m_target_attr.type(),
                                                     CALI_ATTR_SKIP_EVENTS | CALI_ATTR_ASVALUE);
            }
...
struct Aggregator::AggregatorImpl {
    ...
    inline bool is_key(const CaliperMetadataAccessInterface& db,
    ...
    std::shared_ptr<AggregateEntry> get_aggregation_entry(std::vector<const Node*>::const_iterator nodes_begin,
                                                          std::vector<const Node*>::const_iterator nodes_end,
                                                          const std::vector<Entry>&                immediates,
                                                          CaliperMetadataAccessInterface&          db) {

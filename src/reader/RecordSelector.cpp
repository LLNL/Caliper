...
QuerySpec::Condition parse_clause(const std::string& str)
{
...
void RecordSelector::operator() (CaliperMetadataAccessInterface& db, const EntryList& list, SnapshotProcessFn push)
    const
{
    if (mP->pass(db, list))
        push(db, list);
...
std::vector<QuerySpec::Condition> RecordSelector::parse(const std::string& str)
{

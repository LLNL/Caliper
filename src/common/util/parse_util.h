...
std::string read_word(std::istream& is, const char* separators = ",");

/// \brief Reads text within (start_char, end_char) region, skipping
///   over any such regions nested within.
std::string read_nested_text(std::istream& is, char start_char, char end_char);

/// \brief Read character from stream \a is, skipping whitespace.
char read_char(std::istream& is);

inline std::pair<bool, uint64_t> str_to_uint64(const char* str)
{
    uint64_t    ret = 0;
    const char* p   = str;
    for (; *p >= '0' && *p <= '9'; ++p)

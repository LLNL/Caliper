...
class MemstatService
{
    ...
    MemstatService(Caliper* c, int fd) : m_fd { fd }, m_failed { 0 } {
        m_vmsize_attr = c->create_attribute("memstat.vmsize",
                                            CALI_TYPE_UINT,
                                            CALI_ATTR_SCOPE_PROCESS | CALI_ATTR_ASVALUE | CALI_ATTR_AGGREGATABLE);
        m_vmrss_attr = c->create_attribute("memstat.vmrss",
                                           CALI_TYPE_UINT,
                                           CALI_ATTR_SCOPE_PROCESS | CALI_ATTR_ASVALUE | CALI_ATTR_AGGREGATABLE);
        m_vmdata_attr = c->create_attribute("memstat.data",
                                            CALI_TYPE_UINT,
                                            CALI_ATTR_SCOPE_PROCESS | CALI_ATTR_ASVALUE | CALI_ATTR_AGGREGATABLE);
    }
...
public:
    ...
    static void memstat_register(Caliper* c, Channel* channel) {
        ...
        channel->events().snapshot.connect(
            [instance](Caliper* c, Channel*, SnapshotView, SnapshotBuilder& rec) { instance->snapshot_cb(c, rec); });

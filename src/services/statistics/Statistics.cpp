    ...
    void finish_cb(Caliper* c, Channel* chn) {
        ...
        if (chn->id() == 0) {
            ...
            Log(1).stream() << "Global statistics:"
                            ...
                            << "\n    value:                " << n_val << "\n    global:               " << n_global
                            << "\n    hidden:               " << (n_hidden_ref + n_hidden_val) << " (" << n_hidden_ref
                            << " reference, " << n_hidden_val << " value)"
                            << "\n  Number of threads:      " << num_threads << " (max " << max_threads << ")"
                            << std::endl;
        }
    }

    Statistics()
        : num_snapshots(0), num_begin(0), num_end(0), num_set(0), max_snapshot_len(0), num_threads(1), max_threads(1) {
    }
...
public:
    ...
    static void statistics_service_register(Caliper* c, Channel* chn) {
        ...
        chn->events().pre_begin_evt.connect(
            [instance](Caliper* c, Channel* chn, const Attribute&, const Variant&) { ++instance->num_begin; });
        chn->events().pre_end_evt.connect(
            [instance](Caliper* c, Channel* chn, const Attribute&, const Variant&) { ++instance->num_end; });
        chn->events().pre_set_evt.connect(
            [instance](Caliper* c, Channel* chn, const Attribute&, const Variant&) { ++instance->num_set; });
        chn->events().snapshot.connect(
            [instance](Caliper*, Channel*, SnapshotView, SnapshotBuilder& rec) { ++instance->num_snapshots; });
        ...
        if (chn->id() == 0) {
            chn->events().create_thread_evt.connect([instance](Caliper* c, Channel* chn) {
                ...
                instance->max_threads = std::max(instance->num_threads, instance->max_threads);
            });
            chn->events().release_thread_evt.connect([instance](Caliper* c, Channel* chn) { --instance->num_threads; });
        }

...
class TimerService
{
    ...
    TimerService(Caliper* c, Channel* chn) : tstart(clock::now()) {
        ...
        offset_attr = c->create_attribute("time.offset.ns",
                                          CALI_TYPE_UINT,
                                          CALI_ATTR_ASVALUE | CALI_ATTR_SCOPE_THREAD | CALI_ATTR_SKIP_EVENTS,
                                          1,
                                          &unit_attr,
                                          &nsec_val);
        snapshot_duration_attr = c->create_attribute("time.duration.ns",
                                                     CALI_TYPE_UINT,
                                                     CALI_ATTR_ASVALUE | CALI_ATTR_SCOPE_THREAD | CALI_ATTR_SKIP_EVENTS
                                                     ...
                                                     &unit_attr,
                                                     &nsec_val);
        inclusive_duration_attr = c->create_attribute("time.inclusive.duration.ns",
                                                      CALI_TYPE_UINT,
                                                      CALI_ATTR_ASVALUE | CALI_ATTR_SCOPE_THREAD | CALI_ATTR_SKIP_EVENTS
                                                          | CALI_ATTR_AGGREGATABLE,
                                                      1,
                                                      ...
                                                      &nsec_val);
        timerinfo_attr =
            c->create_attribute(std::string("timer.info.") + std::to_string(chn->id()),
                                CALI_TYPE_PTR,

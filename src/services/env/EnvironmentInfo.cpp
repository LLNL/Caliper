...
static const ConfigSet::Entry s_configdata[] = { { "extra",
                                                   CALI_TYPE_STRING,
                                                   "",
...
{
    Attribute cmdline_attr = c->create_attribute("env.cmdline", CALI_TYPE_STRING, CALI_ATTR_GLOBAL);
...
void read_uname(Caliper* c, Channel* chn, ConfigSet& config)
{
    ...
    const struct uname_attr_info_t {
        ...
        const char* u_val;
    } uname_attr_info[] = { { "env.os.sysname", "Windows" },
                            { "env.os.release", "10" },
                            { "env.os.version", "10" },
                            { "env.machine", "x86_64" } };

    for (const uname_attr_info_t& uinfo : uname_attr_info) {
        Attribute attr = c->create_attribute(uinfo.attr_name, CALI_TYPE_STRING, CALI_ATTR_GLOBAL);
    ...
    if (uname(&u) == 0) {
        const struct uname_attr_info_t {
            const char* attr_name;
            const char* u_val;
        } uname_attr_info[] = { { "env.os.sysname", u.sysname },
...
{
    Attribute starttime_attr = c->create_attribute("env.starttime", CALI_TYPE_STRING, CALI_ATTR_GLOBAL);
...
{
    Attribute hostname_attr = c->create_attribute("env.hostname", CALI_TYPE_STRING, CALI_ATTR_SCOPE_PROCESS);
...
void read_extra(Caliper* c, Channel* chn, ConfigSet& config)
{
    ...
    for (string& env : extra_list) {
        ...
        Attribute attr = c->create_attribute("env." + env, CALI_TYPE_STRING, CALI_ATTR_GLOBAL);

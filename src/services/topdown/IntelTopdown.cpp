...
public:
  ...
  compute_toplevel(const std::vector<Entry> &rec) override {
    ...
    double bad_speculation = (v_uops_issued_any.to_double() -
  ...
  compute_backend_bound(const std::vector<Entry> &rec) override {
    ...
    double l2_bound = (v_cycle_activity_stalls_l1d_pending.to_double() -
                      ...
                      clocks;
    double l3_bound = v_cycle_activity_stalls_l2_pending.to_double() *
  ...
  compute_frontend_bound(const std::vector<Entry> &rec) override {
    ...
    Variant v_cpu_clk_unhalted_thread_p =
        get_val_from_rec(rec, "CPU_CLK_THREAD_UNHALTED:THREAD_P");
    Variant v_idq_uops_not_delivered = get_val_from_rec(
        rec, "IDQ_UOPS_NOT_DELIVERED:CYCLES_0_UOPS_DELIV_CORE");
  ...
  compute_bad_speculation(const std::vector<Entry> &rec) override {
    ...
    bool is_incomplete = v_br_misp_retired_all_branches.empty() ||
...
class IntelTopdown {
  ...
  void finish_cb(Caliper *c, Channel *channel) {
    Log(1).stream() << channel->name()
                    << ": topdown: Computed topdown metrics for "
                    << num_top_computed << " records, skipped "
                    << num_top_skipped << std::endl;

    if (Log::verbosity() >= 2) {
      Log(2).stream() << channel->name()
                      << ": topdown: Records processed per topdown level: "
                      << "\n  top:      " << num_top_computed << " computed, "
                      << num_top_skipped << " skipped,"
                      << "\n  bad spec: " << num_bsp_computed << " computed, "
                      << num_bsp_skipped << " skipped,"
                      << "\n  frontend: " << num_bsp_computed << " computed, "
                      << num_bsp_skipped << " skipped,"
                      << "\n  backend:  " << num_bsp_computed << " computed, "
                      << num_bsp_skipped << " skipped." << std::endl;

      const std::map<std::string, int> &counters_not_found =
          m_calculator->get_counters_not_found();
      ...
      if (!counters_not_found.empty()) {
        ...
        for (auto &p : counters_not_found)
          os << "\n  " << p.first << ": " << p.second;
        Log(2).stream() << channel->name()
                        << ": topdown: Counters not found:" << os.str()
                        << std::endl;
      }
    }
  }
...
public:
  ...
  static void intel_topdown_register(Caliper *c, Channel *channel) {
    ...
    channel->events().pre_flush_evt.connect(
        [instance](Caliper *c, Channel *channel, SnapshotView) {
          if (instance->find_counter_attrs(*c))
            ...
            Log(0).stream()
                << channel->name()
                << ": topdown: Could not find counter attributes!" << std::endl;
        });
    ...
    Log(1).stream() << channel->name()
                    << ": Registered topdown service. Level: " << lvlcfg << "."
                    << std::endl;
  }
};
...
CaliperService topdown_service{::IntelTopdown::s_spec,
                               ::IntelTopdown::intel_topdown_register};

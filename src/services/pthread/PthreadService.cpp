...
int cali_pthread_create_wrapper(pthread_t* thread, const pthread_attr_t* attr, void* (*fn)(void*), void* arg) {
...
void pthreadservice_initialize(Caliper* c, Channel* chn) {
    ...
    id_attr = c->create_attribute("pthread.id",
                                  CALI_TYPE_UINT,
                                  CALI_ATTR_SCOPE_THREAD | CALI_ATTR_UNALIGNED,
                                  1,
                                  &subscription_attr,
                                  &v_true);
    master_attr = c->create_attribute("pthread.is_master",
                                      CALI_TYPE_BOOL,
                                      CALI_ATTR_SCOPE_THREAD | CALI_ATTR_UNALIGNED | CALI_ATTR_SKIP_EVENTS);

    chn->events().post_init_evt.connect([](Caliper* c, Channel* chn) { post_init_cb(c, chn); });

...
void print_cupti_error(std::ostream& os, CUptiResult err, const char* func) {
    const char* errstr;
...
bool cleanup_cupti_event_group(CUpti_EventGroup group) {
    CUptiResult res;
...
bool find_event_by_name(const std::string& target_evt_name, CUpti_EventID* ret_evt_id) {
    CUptiResult res;
    ...
    for (CUpti_EventDomainID domain_id : evt_domains) {
        ...
        if (Log::verbosity() >= 2) {
            ...
            Log(2).stream() << "cupti-sampling: Looking for event \"" << target_evt_name << "\" in event domain \""
                            << name << "\"" << std::endl;
        }
        ...
        for (CUpti_EventID evt_id : events) {
            ...
            if (target_evt_name == name) {
                Log(2).stream() << "cupti-sampling:   Found event ID " << evt_id << std::endl;
...
EventSampling::EventSampling() : m_enabled { false } {
}
...
bool EventSampling::setup(Caliper* c, const std::string& event_name) {
    if (!::find_event_by_name(event_name, &m_event_id)) {
        Log(0).stream() << "cupti-sampling: CUpti event \"" << event_name
                        << "\" not found. Disabling CUpti event sampling." << std::endl;
        return false;
    }

    Log(1).stream() << "cupti-sampling: Found CUpti event \"" << event_name << "\" (ID = " << m_event_id << ")"
                    << std::endl;

    Attribute aggr_class_attr = c->get_attribute("class.aggregatable");
    Variant   v_true(true);

    m_event_attr = c->create_attribute(std::string("cupti.event.") + event_name,
                                       CALI_TYPE_UINT,
                                       CALI_ATTR_ASVALUE,
                                       1,
                                       &aggr_class_attr,
...
bool EventSampling::setup(Caliper* c, CUpti_EventID event_id) {
    CUptiResult res;
    ...
    Log(1).stream() << "cupti-sampling: Using CUpti event \"" << name << "\" (ID = " << m_event_id << ")" << std::endl;

    Attribute aggr_class_attr = c->get_attribute("class.aggregatable");
    Variant   v_true(true);

    m_event_attr = c->create_attribute(std::string("cupti.event.") + name,
                                       CALI_TYPE_UINT,
                                       CALI_ATTR_ASVALUE,
                                       1,
                                       &aggr_class_attr,
...
bool EventSampling::enable_sampling_for_context(CUcontext context) {
    SamplingInfo info;
    CUptiResult  res;
    ...
    if (Log::verbosity() >= 2) {
        ...
        Log(2).stream() << "cupti-sampling: Creating event group on context " << context_id << " for event "
                        << m_event_id << std::endl;
    }
    ...
    {
        std::lock_guard<std::mutex> g(m_sampling_mtx);
    ...
    if (Log::verbosity() >= 2) {
        ...
        Log(2).stream() << "cupti-sampling: Started event sampling on context " << context_id << std::endl;
    }

    return true;
}

bool EventSampling::disable_sampling_for_context(CUcontext context) {
    auto it = std::find_if(m_sampling_info.begin(), m_sampling_info.end(), [context](const SamplingInfo& info) {
        return info.context == context;
    });

    if (it == m_sampling_info.end()) {
        Log(0).stream() << "cupti-sampling: stop sampling for unknown context " << context << std::endl;
        return false;
    }
...
void EventSampling::snapshot(Caliper* c, SnapshotBuilder& snapshot) {
    ++m_num_snapshots;
    ...
    CUptiResult res = cuptiEventGroupReadEvent(group, CUPTI_EVENT_READ_FLAG_NONE, m_event_id, &bytes_read, &val);
...
std::ostream& EventSampling::print_statistics(std::ostream& os) {
    os << "cupti-sampling: " << m_num_snapshots << " total snapshots, " << m_num_reads << " cupti events read."
       << std::endl;

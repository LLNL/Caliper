...
class CuptiTraceService
{
    ...
    static void print_cupti_error(std::ostream& os, CUptiResult err, const char* func) {
        const char* errstr;
    ...
    void add_completed_buffer(ActivityBuffer* acb, size_t dropped) {
        ...
        std::lock_guard<std::mutex> g(retired_buffers_list_lock);
    ...
    buffer_completed(CUcontext ctx, uint32_t stream, uint8_t* buffer, size_t size, size_t valid_size) {
        ActivityBuffer* acb = new ActivityBuffer(ctx, stream, buffer, size, valid_size);
    ...
    const char* get_memcpy_kind_string(CUpti_ActivityMemcpyKind kind) {
        switch (kind) {
    ...
    const char* get_uvm_counter_kind_string(CUpti_ActivityUnifiedMemoryCounterKind kind) {
        switch (kind) {
    ...
    const char* get_uvm_migration_cause_string(CUpti_ActivityUnifiedMemoryMigrationCause cause) {
        switch (cause) {
    ...
    const char* get_uvm_access_type_string(CUpti_ActivityUnifiedMemoryAccessType access) {
        switch (access) {
    ...
    size_t flush_record(CUpti_Activity*           rec,
                        correlation_id_map_t&     correlation_map,
                        Caliper*                  c,
                        const std::vector<Entry>& irec,
                        SnapshotFlushFn           proc_fn) {
        switch (rec->kind) {
        case CUPTI_ACTIVITY_KIND_DEVICE:
            {
                CUpti_ActivityDevice2* device = reinterpret_cast<CUpti_ActivityDevice2*>(rec);
...

                {
                    ...
                    std::copy(device->uuid.bytes,
                              device->uuid.bytes + sizeof(device->uuid.bytes),
        ...
        case CUPTI_ACTIVITY_KIND_EXTERNAL_CORRELATION:
            {
                CUpti_ActivityExternalCorrelation* exco = reinterpret_cast<CUpti_ActivityExternalCorrelation*>(rec);
        ...
        case CUPTI_ACTIVITY_KIND_MEMCPY:
            {
                CUpti_ActivityMemcpy* memcpy = reinterpret_cast<CUpti_ActivityMemcpy*>(rec);
...

                {
                    ...
                    if (it != device_info_map.end())
                        parent = c->make_tree_entry(device_uuid_attr,
                ...
                Variant   data[6] = { Variant("memcpy"),
                                      Variant(get_memcpy_kind_string(
        ...
        case CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL:
            {
                CUpti_ActivityKernel4* kernel = reinterpret_cast<CUpti_ActivityKernel4*>(rec);
        ...
        case CUPTI_ACTIVITY_KIND_UNIFIED_MEMORY_COUNTER:
            {
                CUpti_ActivityUnifiedMemoryCounter2* uvm = reinterpret_cast<CUpti_ActivityUnifiedMemoryCounter2*>(rec);
    ...
    size_t flush_buffer(ActivityBuffer* acb, Caliper* c, const std::vector<Entry>& irec, SnapshotFlushFn proc_fn) {
        if (!(acb->valid_size > 0))
            return 0;
    ...
    void finish_cb(Caliper* c, Channel* chn) {
        ...
        if (flush_on_snapshot)
            Log(1).stream() << chn->name() << ": cuptitrace: Triggered " << num_snapshot_flushes << " snapshot flushes."
                            << std::endl;
        if (num_dropped_records > 0)
            Log(1).stream() << chn->name() << ": cuptitrace: Dropped " << num_dropped_records << " records."
                            << std::endl;

        unitfmt_result bytes_reserved = unitfmt(num_buffers_completed * buffer_size, unitfmt_bytes);
        unitfmt_result bytes_used = unitfmt(buffer_size_used, unitfmt_bytes);

        Log(1).stream() << chn->name() << ": cuptitrace: Allocated " << num_buffers_allocated << " buffers ("
                        << bytes_reserved.val << bytes_reserved.symbol << " reserved, " << bytes_used.val
                        << bytes_used.symbol << " used). " << num_buffers_completed << " buffers completed, "
        ...
        Log(2).stream() << chn->name() << ": cuptitrace: Processed CUpti activity records:"
                        ...
                        << "\n  unknown records:     " << num_unknown_recs << std::endl;

        Log(2).stream() << chn->name() << ": cuptitrace: " << num_correlations_found << " context correlations found, "
    ...
    void configure_uvm_recording(const ConfigSet& config) {
        ...
        if (res != CUPTI_SUCCESS) {
            ...
            Log(0).stream() << "cuptitrace: cuptiActivityConfigureUnifiedMemoryCounter: " << errstr << std::endl;
        }
    }
    ...
    void enable_cupti_activities(const ConfigSet& config) {
        ...
        } activity_map[] = { { "correlation", CUPTI_ACTIVITY_KIND_EXTERNAL_CORRELATION },
        ...
        for (const activity_map_t* act = activity_map; act && act->name; ++act) {
            ...
            if (res != CUPTI_SUCCESS) {
                ...
                Log(0).stream() << "cuptitrace: cuptiActivityEnable (" << act->name << "): " << errstr << std::endl;
            }
        }
    ...
    void snapshot_cb(Caliper* c, Channel* chn, const SnapshotView, SnapshotBuilder& snapshot) {
        ...
        if (record_host_duration)
            snapshot.append(Entry(duration_attr, Variant(cali_make_variant_from_uint(timestamp - v_prev.to_uint()))));
    }

    void snapshot_flush_activities_cb(Caliper*         c,
                                      Channel*         channel,
    ...
    void post_init_cb(Caliper* c, Channel* chn) {
        ...
        chn->events().pre_finish_evt.connect([](Caliper* c, Channel* chn) { s_instance->pre_finish_cb(c, chn); });
    ...
    CuptiTraceService(Caliper* c, Channel* chn) {
        ...
        activity_start_attr =
            c->create_attribute("cupti.activity.start", CALI_TYPE_UINT, CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS);
        activity_end_attr =
            c->create_attribute("cupti.activity.end", CALI_TYPE_UINT, CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS);
        activity_duration_attr = c->create_attribute("cupti.activity.duration",
                                                     CALI_TYPE_UINT,
        ...
        memcpy_kind_attr =
            c->create_attribute("cupti.memcpy.kind", CALI_TYPE_STRING, CALI_ATTR_DEFAULT | CALI_ATTR_SKIP_EVENTS);
        memcpy_bytes_attr = c->create_attribute("cupti.memcpy.bytes",
                                                CALI_TYPE_UINT,
                                                CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        starttime_attr =
            c->create_attribute("cupti.starttime", CALI_TYPE_UINT, CALI_ATTR_SCOPE_PROCESS | CALI_ATTR_SKIP_EVENTS);
        device_uuid_attr =
            c->create_attribute("cupti.device.uuid", CALI_TYPE_STRING, CALI_ATTR_DEFAULT | CALI_ATTR_SKIP_EVENTS);
        fault_address_attr = c->create_attribute("cupti.fault.addr",
                                                 CALI_TYPE_ADDR,
                                                 ...
                                                 &true_val);
        uvm_kind_attr =
            c->create_attribute("cupti.uvm.kind", CALI_TYPE_STRING, CALI_ATTR_DEFAULT | CALI_ATTR_SKIP_EVENTS);
        uvm_bytes_attr = c->create_attribute("cupti.uvm.bytes",
                                             CALI_TYPE_UINT,
                                             CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        uvm_pagefault_groups_attr =
            c->create_attribute("cupti.uvm.pagefault.groups",
                                CALI_TYPE_UINT,
                                CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        uvm_migration_cause_attr = c->create_attribute("cupti.uvm.migration.cause",
                                                       CALI_TYPE_STRING,
        ...
        if (record_host_duration || record_host_timestamp) {
            ...
            timestamp_attr =
                c->create_attribute("cupti.timestamp",
                                    CALI_TYPE_UINT,
                                    CALI_ATTR_SCOPE_THREAD | CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | hide_offset);
            duration_attr = c->create_attribute("cupti.host.duration",
                                                CALI_TYPE_UINT,
...
public:
    ...
    static void cuptitrace_initialize(Caliper* c, Channel* chn) {
        if (s_instance) {
            Log(0).stream() << chn->name() << ": cuptitrace service is already initialized!" << std::endl;
            return;
        }

        s_instance = new CuptiTraceService(c, chn);

        chn->events().post_init_evt.connect([](Caliper* c, Channel* chn) { s_instance->post_init_cb(c, chn); });
    }
};

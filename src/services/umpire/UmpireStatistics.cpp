...
class UmpireService
{
    ...
    void record_global_highwatermarks(Caliper* c, ChannelBody* chB)
    {
        ...
        for (const auto& s : rm.getAllocatorNames()) {
            Attribute attr = c->create_attribute(
                std::string("umpire.highwatermark.") + s,
                CALI_TYPE_UINT,
    ...
    void create_attributes(Caliper* c)
    {
        ...
        m_alloc_actual_size_attr = c->create_attribute(
            "umpire.alloc.actual.size",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );
        m_alloc_hwm_attr = c->create_attribute(
            "umpire.alloc.highwatermark",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );
        m_alloc_count_attr = c->create_attribute(
            "umpire.alloc.count",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );
        m_total_size_attr = c->create_attribute(
            "umpire.total.size",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );
        m_total_count_attr = c->create_attribute(
            "umpire.total.count",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );
        m_total_hwm_attr = c->create_attribute(
            "umpire.total.hwm",
            CALI_TYPE_UINT,
            CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE
        );

...
class UmpireService
{
    ...
    bool is_tracked_allocator(const std::string& s) {
        return m_filter.empty() || std::find(m_filter.begin(), m_filter.end(), s) != m_filter.end();
    }

    void process_allocator(Caliper*           c,
                           Channel*           channel,
        ...
        Attribute attr[] = { m_alloc_name_attr,
                             m_alloc_actual_size_attr,
                             m_alloc_current_size_attr,
                             m_alloc_hwm_attr,
                             m_alloc_count_attr };
        Variant   data[] = { Variant(CALI_TYPE_STRING, name.data(), name.size()),
                             Variant(actual_size),
    ...
    void record_global_highwatermarks(Caliper* c, Channel* channel) {
        ...
        for (const auto& s : rm.getAllocatorNames()) {
            Attribute attr = c->create_attribute(std::string("umpire.highwatermark.") + s,
                                                 CALI_TYPE_UINT,
    ...
    void finish_cb(Caliper* c, Channel* channel) {
        Log(1).stream() << channel->name() << ": Finished Umpire service" << std::endl;
    }

    void create_attributes(Caliper* c) {
        m_alloc_name_attr = c->create_attribute("umpire.alloc.name", CALI_TYPE_STRING, CALI_ATTR_SKIP_EVENTS);
        m_alloc_current_size_attr =
        ...
        m_alloc_actual_size_attr =
            c->create_attribute("umpire.alloc.actual.size",
                                CALI_TYPE_UINT,
                                CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        m_alloc_hwm_attr = c->create_attribute("umpire.alloc.highwatermark",
                                               CALI_TYPE_UINT,
                                               CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        m_alloc_count_attr = c->create_attribute("umpire.alloc.count",
                                                 CALI_TYPE_UINT,
                                                 CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        m_total_size_attr = c->create_attribute("umpire.total.size",
                                                CALI_TYPE_UINT,
                                                CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        m_total_count_attr = c->create_attribute("umpire.total.count",
                                                 CALI_TYPE_UINT,
                                                 CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
        m_total_hwm_attr = c->create_attribute("umpire.total.hwm",
                                               CALI_TYPE_UINT,
                                               CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS | CALI_ATTR_AGGREGATABLE);
    }
    ...
    UmpireService(Caliper* c, Channel* channel) : m_root_node(CALI_INV_ID, CALI_INV_ID, Variant()) {
        ...
        m_filter = config.get("allocator_filter").to_stringlist();
...
public:
    ...
    static void umpire_register(Caliper* c, Channel* channel) {
        ...
        channel->events().post_init_evt.connect(
            [instance](Caliper* c, Channel*) { instance->m_timestamp_attr = c->get_attribute("time.offset"); });

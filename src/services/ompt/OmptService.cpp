...
struct OmptAPI {
    ...
    bool init(ompt_function_lookup_t lookup) {
...
void cb_implicit_task(ompt_scope_endpoint_t endpoint,
                      ompt_data_t*,
                      ompt_data_t*,
                      unsigned int par,
                      unsigned int index,
...
void cb_work(int wstype, ompt_scope_endpoint_t endpoint, ompt_data_t*, ompt_data_t*, uint64_t, const void*) {
    const char* work_region_names[] = { "UNKNOWN",         "loop",         "sections",
                                        "single_executor", "single_other", "workshare",
                                        "distribute",      "taskloop",     "scope" };
...
void cb_sync_region(int kind, ompt_scope_endpoint_t endpoint, ompt_data_t*, ompt_data_t*, const void*) {
    const char* sync_region_names[] = {
        "UNKNOWN",      "barrier",   "barrier_implicit", "barrier_explicit",           "barrier_implementation",
        "taskwait",     "taskgroup", "reduction",        "barrier_implicit_workshare", "barrier_implicit_parallel",
        "barrier_teams"
    };
...
void setup_ompt_callbacks() {
    const struct callback_info {
        ompt_callbacks_t cb;
        ompt_callback_t  fn;
    } callbacks[] = { { ompt_callback_thread_begin, reinterpret_cast<ompt_callback_t>(cb_thread_begin) },
...
void create_attributes(Caliper* c) {
    ...
    region_attr =
        c->create_attribute("omp.parallel", CALI_TYPE_UINT, CALI_ATTR_SCOPE_THREAD, 1, &subscription_attr, &v_true);
    thread_type_attr = c->create_attribute("omp.thread.type",
                                           CALI_TYPE_STRING,
                                           CALI_ATTR_SCOPE_THREAD | CALI_ATTR_UNALIGNED,
                                           1,
                                           &subscription_attr,
                                           &v_true);
    sync_attr =
        c->create_attribute("omp.sync", CALI_TYPE_STRING, CALI_ATTR_SCOPE_THREAD, 1, &subscription_attr, &v_true);
    work_attr =
        c->create_attribute("omp.work", CALI_TYPE_STRING, CALI_ATTR_SCOPE_THREAD, 1, &subscription_attr, &v_true);
    state_attr = c->create_attribute("omp.state", CALI_TYPE_STRING, CALI_ATTR_SCOPE_THREAD | CALI_ATTR_SKIP_EVENTS);
    proc_id_attr = c->create_attribute("omp.proc.id",
                                       CALI_TYPE_INT,
                                       CALI_ATTR_SCOPE_THREAD | CALI_ATTR_UNALIGNED | CALI_ATTR_SKIP_EVENTS);
    thread_id_attr = c->create_attribute("omp.thread.id",
                                         CALI_TYPE_INT,
                                         CALI_ATTR_SKIP_EVENTS | CALI_ATTR_UNALIGNED | CALI_ATTR_SCOPE_THREAD);
    num_threads_attr = c->create_attribute("omp.num.threads",
                                           CALI_TYPE_INT,
                                           CALI_ATTR_SCOPE_THREAD | CALI_ATTR_UNALIGNED | CALI_ATTR_SKIP_EVENTS);
}
...
void register_ompt_service(Caliper* c, Channel* channel) {
    ...
    Log(1).stream() << channel->name() << ": "
                    << "Registered OMPT service" << std::endl;
}
...
ompt_start_tool_result_t* ompt_start_tool(unsigned omp_version, const char* runtime_version) {
    ...
    if (Log::verbosity() >= 2)
        Log(2).stream() << "OMPT is available. Using " << runtime_version
                        << ". OMPT requested: " << (use_ompt ? "Yes" : "No") << std::endl;

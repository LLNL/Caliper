...
void print_papi_error(const char *function, int code) {
  Log(0).stream() << "papi: Error: " << function << ": " << PAPI_strerror(code)
                  << std::endl;
}
...
class PapiService {
  ...
  typedef std::map<int, std::shared_ptr<event_group_t>> eventset_map_t;
  ...
  struct ThreadInfo {
    ...
    ThreadInfo() : next(nullptr), prev(nullptr) {}
  };
  ...
  bool setup_event_info(Caliper *c, const std::vector<std::string> &eventlist) {
    ...
    for (auto &name : eventlist) {
      ...
      if (it == m_event_groups.end()) {
        auto p = m_event_groups.emplace(std::make_pair(
            component, std::shared_ptr<event_group_t>(new event_group_t)));
        it = p.first;
      }
    ...
    Log(2).stream() << "papi: Found " << count << " event codes for "
                    << m_event_groups.size() << " PAPI component(s)"
                    << std::endl;
  ...
  bool setup_thread_eventsets(Caliper *c) {
    ...
    for (auto &p : m_event_groups) {
      ...
      if (Log::verbosity() >= 2) {
        Log(2).stream() << "papi: Creating eventset with "
                        << p.second->codes.size() << " events for component "
                        << p.first << " ("
      ...
      if (!m_disable_multiplex && cpi &&
          (num > 4 /* magic number for Intel counter support :-( */ ||
           m_enable_multiplex)) {
        if (Log::verbosity() >= 2)
          Log(2).stream()
              << "papi: Initializing multiplex support for component "
    ...
    if (ok) {
      ...
      {
        std::lock_guard<util::spinlock> g(m_thread_lock);
  ...
  void read_events(int eventset, const event_group_t &grp,
                   SnapshotBuilder &rec) {
  ...
  void finish_thread_eventsets(Caliper *c) {
    ThreadInfo *td = get_thread_info(c);
  ...
  void snapshot(Caliper *c, SnapshotBuilder &rec) {
    ThreadInfo *td = get_thread_info(c);
  ...
  void finish(Caliper *c, Channel *channel) {
    ...
    unsigned errors = m_num_event_mismatch + m_num_failed_acquire +
                      m_num_failed_read + m_num_failed_reset +
                      m_num_failed_start;

    if (errors > 0)
      Log(0).stream() << channel->name() << ": papi: " << m_num_failed_acquire
                      << " failed thread data accesses, " << m_num_failed_read
                      << " failed reads, " << m_num_failed_reset
                      << " failed resets, " << m_num_failed_start
                      << " failed starts, " << m_num_event_mismatch
                      << " event count mismatches." << std::endl;

    Log(1).stream() << channel->name() << ": papi: Created " << m_num_eventsets
                    << " PAPI event set(s) on " << m_num_threads
                    << " thread(s)." << std::endl;
  }

  PapiService(Caliper *c, Channel *channel)
      : m_enable_multiplex(false), m_disable_multiplex(false),
        m_num_eventsets(0), m_num_event_mismatch(0), m_num_failed_acquire(0),
        m_num_failed_read(0), m_num_failed_reset(0), m_num_failed_start(0),
        m_num_threads(0), m_thread_list(nullptr) {
    m_thread_attr = c->create_attribute(
        std::string("papi.data.") + std::to_string(channel->id()),
        CALI_TYPE_PTR,
        CALI_ATTR_SCOPE_THREAD | CALI_ATTR_HIDDEN | CALI_ATTR_ASVALUE |
  ...
  static bool init_papi_library() {
    ...
    if (ret != PAPI_VER_CURRENT && ret > 0) {
      Log(0).stream() << "papi: PAPI version mismatch: found " << ret
                      << ", expected " << PAPI_VER_CURRENT << std::endl;
      return false;
    }
    ...
    if (PAPI_is_initialized() == PAPI_NOT_INITED) {
      Log(0).stream() << "papi: PAPI library could not be initialized"
                      << std::endl;
      return false;
    }
...
public:
  ...
  static void register_papi(Caliper *c, Channel *channel) {
    ...
    if (!(instance->setup_event_info(c, eventlist) &&
          instance->setup_thread_eventsets(c))) {
      Log(0).stream()
          << channel->name()
    ...
    Log(1).stream() << channel->name() << ": Registered papi service"
                    << std::endl;
  }
};
...
CaliperService papi_service = {::PapiService::s_spec,
                               ::PapiService::register_papi};

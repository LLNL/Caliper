...
class RocProfilerService
{
    ...
    void snapshot_cb(Caliper* c, Channel* channel, SnapshotView trigger_info, SnapshotBuilder& snapshot) {
        ...
        snapshot.append(Entry(m_host_duration_attr,
                              cali_make_variant_from_uint(timestamp - v_prev.to_uint())));
    }
    ...
    void pre_finish_cb(Caliper* c, Channel* channel) {
        ...
        Log(1).stream() << channel->name()
            << ": rocprofiler: wrote " << m_num_activity_records << " activity records\n";
    }
    ...
    static void register_rocprofiler(Caliper* c, Channel* channel) {
        if (s_instance) {
            Log(0).stream() << channel->name()
                            << ": rocprofiler service is already active, disabling!"
                            << std::endl;
        }

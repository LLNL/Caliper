...
int set_external_correlation_id(rocprofiler_thread_id_t /*thr_id*/,
...
class RocProfilerService
{
    ...
    void create_attributes(Caliper* c) {
        ...
        m_host_timestamp_attr = c->create_attribute("rocm.host.timestamp",
                                                    CALI_TYPE_UINT,
    ...
    static void tool_tracing_callback(rocprofiler_context_id_t      context,
        ...
        for (size_t i = 0; i < num_headers; ++i) {
            ...
            if (header->category == ROCPROFILER_BUFFER_CATEGORY_TRACING
                ...
                uint64_t    len;
                ROCPROFILER_CALL(rocprofiler_query_buffer_tracing_kind_operation_name(record->kind,
                                                                                      record->operation,
                                                                                      &activity_name,
                                                                                      &len));
                ...
                const Variant data[] = { Variant(CALI_TYPE_STRING, activity_name, len),
                ...
                s_instance->m_channel.events().process_snapshot(&c,
                                                                &s_instance->m_channel,
                                                                SnapshotView(),
                                                                snapshot.view());
    ...
    static void tool_api_cb(rocprofiler_callback_tracing_record_t record,
        ...
        if (record.kind == ROCPROFILER_CALLBACK_TRACING_CODE_OBJECT) {
            if (record.operation == ROCPROFILER_CODE_OBJECT_DEVICE_KERNEL_SYMBOL_REGISTER
                ...
                auto* data = static_cast<rocprofiler_callback_tracing_code_object_kernel_symbol_register_data_t*>(
                    record.payload);
...
public:
    ...
    static int tool_init(rocprofiler_client_finalize_t fini_func, void* tool_data) {
        ...
        ROCPROFILER_CALL(rocprofiler_configure_callback_tracing_service(hip_api_ctx,
                                                                        ROCPROFILER_CALLBACK_TRACING_HIP_RUNTIME_API,
                                                                        nullptr,
                                                                        0,
                                                                        tool_api_cb,
                                                                        nullptr));
        ROCPROFILER_CALL(rocprofiler_configure_callback_tracing_service(rocprofiler_ctx,
        ...
        ROCPROFILER_CALL(rocprofiler_create_buffer(activity_ctx,
                                                   16 * 1024 * 1024,
                                                   15 * 1024 * 1024,
        ...
        ROCPROFILER_CALL(rocprofiler_configure_buffer_tracing_service(activity_ctx,
...
*/
        auto external_corr_id_request_kinds = make_array(ROCPROFILER_EXTERNAL_CORRELATION_REQUEST_KERNEL_DISPATCH,
                                                         ROCPROFILER_EXTERNAL_CORRELATION_REQUEST_MEMORY_COPY);

        ROCPROFILER_CALL(
            rocprofiler_configure_external_correlation_id_request_service(activity_ctx,

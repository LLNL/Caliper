...
class VariorumService
{
    ...
    void finish_cb(Caliper* c, Channel* channel) {
        ...
        if (m_num_errors > 0) {
            Log(0).stream() << channel->name() << ": variorum: " << m_num_errors << " measurement errors!" << std::endl;
        }
    }
    ...
    MeasurementInfo create_measurement_info(Caliper* c, Channel* channel, const std::string& domain) {
        ...
        for (auto& domain : domainList) {
            m.value_attr = c->create_attribute(std::string("variorum.val.") + domain,
                                               CALI_TYPE_UINT,
            ...
            // automatically.
            m.delta_attr = c->create_attribute(std::string("variorum.") + domain,
                                               CALI_TYPE_UINT,
                                               CALI_ATTR_SCOPE_THREAD | CALI_ATTR_ASVALUE | CALI_ATTR_SKIP_EVENTS
                                                   | CALI_ATTR_AGGREGATABLE);
            ...
            m.prval_attr = c->create_attribute(std::string("variorum.pv.") + std::to_string(channel->id()) + domain,
                                               CALI_TYPE_UINT,
                                               CALI_ATTR_SCOPE_THREAD | CALI_ATTR_ASVALUE | CALI_ATTR_HIDDEN
    ...
    VariorumService(Caliper* c, Channel* channel) : m_num_errors(0) {
...
public:
    ...
    static void register_variorum(Caliper* c, Channel* channel) {
        ...
        channel->events().post_init_evt.connect(
            [instance](Caliper* c, Channel* channel) { instance->post_init_cb(c, channel); });
        ...
        Log(1).stream() << channel->name() << ": Registered variorum service" << std::endl;
    }
};

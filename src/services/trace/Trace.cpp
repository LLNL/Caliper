...
public:
    ...
    static void trace_register(Caliper* c, Channel* chn) {
        ...
        chn->events().create_thread_evt.connect(
            [instance](Caliper* c, Channel* chn) { instance->create_thread_cb(c, chn); });
        chn->events().release_thread_evt.connect(
            [instance](Caliper* c, Channel* chn) { instance->release_thread_cb(c, chn); });
        ...
        chn->events().flush_evt.connect(
            [instance](Caliper* c, Channel* chn, SnapshotView, SnapshotFlushFn fn) { instance->flush_cb(c, chn, fn); });

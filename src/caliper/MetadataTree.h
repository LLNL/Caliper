...
public:
    ...
    Node* get_path(const Attribute& attr, size_t n, const Variant data[], Node* parent);

    /// \brief Get or construct a path in the tree under parent with
    ///   the data of the nodes given in the nodelist in the order of that list
    Node* get_path(size_t n, const Node* nodelist[], Node* parent);
    ...
    Node* root() const { return &(mG.load()->root); }

    Node* type_node(cali_attr_type type) const { return mG.load()->type_nodes[type]; };

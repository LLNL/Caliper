...
const char* trigger_grp[] = { "alloc",        "cuptitrace",     "event",   "libpfm",
                              "loop_monitor", "region_monitor", "sampler", nullptr };
...
struct service_group_t {
    ...
    const char** services;
} service_groups[] = { { "snapshot trigger", trigger_grp },
...
void check_service_dependency(const service_group_t           dependent,
    ...
    Log(1).stream() << "Config check: Warning: " << dependent.group_name << " service \"" << *dept << "\" requires "
                    << dependency.group_name << " services, but none are active.\n     Add "
...
void check_services(const std::vector<std::string>& services) {
    ...
    for (const ServiceDependency& d : dependencies)
        check_service_dependency(service_groups[d.dept], service_groups[d.depcy], services);
}
...
void config_sanity_check(const char* channel, RuntimeConfig cfg) {
    ...
    if (services.empty()) {
        Log(1).stream() << channel << ": No services enabled, " << channel << " channel will not record data."
                        << std::endl;
        return;
    }

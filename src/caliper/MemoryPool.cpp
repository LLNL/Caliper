...
struct MemoryPool::MemoryPoolImpl {
    ...
    {
        std::lock_guard<util::spinlock> g(m_lock);
    ...
    {
        unitfmt_result bytes_reserved = unitfmt(m_total_reserved, unitfmt_bytes);
        unitfmt_result bytes_used     = unitfmt(m_total_used, unitfmt_bytes);

        os << "Metadata memory pool: " << bytes_reserved.val << " " << bytes_reserved.symbol << " reserved, "
...
MemoryPool::MemoryPool() : mP { new MemoryPoolImpl }
{}

MemoryPool::MemoryPool(size_t bytes) : mP { new MemoryPoolImpl }
{
    mP->expand(bytes);
}

...
ChannelController* make_basic_channel_controller(const char*                   name,
...
std::pair<bool, StringConverter> find_key_in_json(const std::vector<std::string>&               path,
...
bool ConfigManager::Options::is_set(const char* option) const {
    ...
    return std::find_if(mP->args.begin(),
                        mP->args.end(),
...
struct ConfigManager::ConfigManagerImpl {
    ...
    std::vector<std::pair<const std::shared_ptr<config_spec_t>, arglist_t>> parse_configstring(

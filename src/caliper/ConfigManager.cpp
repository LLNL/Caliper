...
ChannelController *
make_basic_channel_controller(const char *name, const config_map_t &initial_cfg,
                              const ConfigManager::Options &opts) {
  class BasicController : public ChannelController {
  public:
    BasicController(const char *name, const config_map_t &initial_cfg,
                    const ConfigManager::Options &opts)
...
merge_new_elements(ConfigManager::arglist_t &to,
                   const ConfigManager::arglist_t &from) {
  for (auto &p : from) {
    ...
    if (it == to.end() ||
        p.first == "metadata") // hacky but we want to allow multiple entries
                               // for metadata
      to.push_back(p);
  }
...
std::string find_or(const std::map<std::string, std::string> &m,
...
class ConfigManager::OptionSpec {
  ...
  void parse_select(const std::vector<StringConverter> &list,
                    query_arg_t &qarg) {
    for (const StringConverter &sc : list) {
  ...
  void parse_query_args(const std::vector<StringConverter> &list,
                        option_spec_t &opt) {
    for (const StringConverter &sc : list) {
  ...
  void parse_config(const std::map<std::string, StringConverter> &dict,
                    option_spec_t &opt) {
    for (auto p : dict)
...
public:
  ...
  void add(const OptionSpec &other,
    ...
    for (const auto &p : other.data)
      if (std::find(categories.begin(), categories.end(), p.second.category) !=
          categories.end())
        data.insert(p);
  }
    ...
    for (auto &p : list) {
      ...
      if (m_error) {
        m_error_msg = std::string("option spec: ") +
                      util::clamp_string(p.to_string(), 32) + m_error_msg;
        break;
      }
    }
  }
  ...
  void filter_unavailable_options(const std::vector<std::string> &in) {
    ...
    while (it != data.end()) {
      ...
      if (std::includes(available.begin(), available.end(), required.begin(),
                        required.end()))
        ++it;
      else
        it = data.erase(it);
...
struct ConfigManager::Options::OptionsImpl {
  ...
  std::string check() const {
    ...
    for (const auto &arg : args) {
      ...
      if (it->second.type == "bool") {
        ...
        if (!ok)
          return std::string("Invalid value \"") + arg.second +
                 std::string("\" for ") + arg.first;
      }
    }
    ...
    for (const std::string &opt : enabled_options) {
      ...
      for (const std::string &required_service : o_it->second.services)
        if (std::find(slist.begin(), slist.end(), required_service) ==
            slist.end())
          return required_service + std::string(" service required for ") +
                 o_it->first + std::string(" option is not available");
    }
  ...
  std::string services(const std::string &in) {
    ...
    for (const std::string &opt : enabled_options) {
      ...
      vec.insert(vec.end(), it->second.services.begin(),
                 it->second.services.end());
    }
  ...
  std::string build_query(const char *level,
                          const std::map<std::string, std::string> &in) const {
  ...
  OptionsImpl(const OptionSpec &s, const arglist_t &a) : spec(s), args(a) {
...
ConfigManager::Options::~Options() { mP.reset(); }
...
StringConverter ConfigManager::Options::get(const char *option,
...
std::string ConfigManager::Options::check() const { return mP->check(); }

void ConfigManager::Options::update_channel_config(config_map_t &config) const {
  mP->update_channel_config(config);
}

void ConfigManager::Options::update_channel_metadata(
    info_map_t &metadata) const {
  mP->update_channel_metadata(metadata);
}

std::string ConfigManager::Options::build_query(
    const char *level, const std::map<std::string, std::string> &in) const {
...
struct ConfigManager::ConfigManagerImpl {
  ...
  void add_config_spec(const char *jsonspec, CreateConfigFn create,
                       CheckArgsFn check, bool ignore_existing) {
    config_spec_t spec;
    bool ok = false;

    spec.json = jsonspec;
    spec.create =
        (create == nullptr) ? ::make_basic_channel_controller : create;
    spec.check_args = check;

    auto dict = StringConverter(spec.json).rec_dict(&ok);

    if (!ok) {
      set_error(std::string("spec parse error: ") +
                util::clamp_string(spec.json, 48));
      return;
    }

    auto it = dict.find("name");
    if (it == dict.end()) {
      set_error(std::string("'name' missing in spec: ") +
                util::clamp_string(spec.json, 48));
      return;
    }
    ...
    if (cfg_srvcs.size() > 0)
      spec.initial_cfg["CALI_SERVICES_ENABLE"].append(
          ::join_stringlist(cfg_srvcs));
  ...
  void add_global_option_specs(const char *json) {
    ...
    if (!ok) {
      ...
      set_error(std::string("parse error: ") +
                util::clamp_string(builtin_option_specs, 48));
  ...
  void import_builtin_config_specs() {
    ...
    for (const ConfigInfo &s :
         ::ConfigSpecManager::instance()->get_config_specs())
      add_config_spec(s.spec, s.create, s.check_args,
                      true /* ignore existing */);
  }
  ...
  void parse_json_content(const std::string &json) {
    ...
    if (ok) {
      ...
      if (it != dict.end()) {
        ...
        if (!ok)
          set_error(std::string("parse error: ") +
                    util::clamp_string(it->second.to_string(), 48));
      }
  ...
  //   Returns found configs with their args. Also updates the default
  //   parameters list
  ...
  arglist_t add_default_parameters(arglist_t &args,
                                   const config_spec_t &spec) const {
  ...
  ChannelList parse(const char *config_string) {
    ...
    for (auto cfg : configs) {
      Options opts(options_for_config(*cfg.first),
                   add_default_parameters(cfg.second, *cfg.first));
      ...
      if (m_error)
        ...
        ret.emplace_back((cfg.first->create)(cfg.first->name.c_str(),
                                             cfg.first->initial_cfg, opts));
    }
  ...
  std::string get_documentation_for_spec(const char *name) const {
    ...
    if (it == m_spec.end()) {
      ...
      auto optdescrmap =
          options_for_config(*it->second).get_option_descriptions();
...
ConfigManager::~ConfigManager() { mP.reset(); }

void ConfigManager::add_config_spec(const ConfigInfo &info) {
  mP->add_config_spec(info.spec, info.create, info.check_args,
                      false /* treat existing spec as error */);
}
...
void ConfigManager::load(const char *filename) { mP->load_file(filename); }

bool ConfigManager::add(const char *config_str) {
  mP->add(config_str);

  if (!mP->m_extra_vars.empty())
    mP->set_error("Unknown config or parameter: " +
                  mP->m_extra_vars.begin()->first);
...
bool ConfigManager::error() const { return mP->m_error; }

std::string ConfigManager::error_msg() const { return mP->m_error_msg; }
...
void ConfigManager::set_default_parameter_for_config(const char *config,
                                                     const char *key,
                                                     const char *value) {
  mP->m_default_parameters_for_spec[config].push_back(
...
std::string ConfigManager::check(const char *configstr,
                                 bool allow_extra_kv_pairs) const {
  ...
  for (auto cfg : configs) {
    Options opts(tmpP.options_for_config(*cfg.first),
                 tmpP.add_default_parameters(cfg.second, *cfg.first));
  ...
  if (!allow_extra_kv_pairs && !tmpP.m_extra_vars.empty())
    tmpP.set_error("Unknown config or parameter: " +
                   tmpP.m_extra_vars.begin()->first);
...
std::string ConfigManager::check_config_string(const char *configstr,
                                               bool allow_extra_kv_pairs) {

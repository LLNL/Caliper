...
const Args::Table option_table[] = {
    ...
    { "reuse", "reuse-statistics", 'r', false, "Print tree data reuse statistics", nullptr },
...
public:
    ...
    void print_results(CaliperMetadataAccessInterface& db, ostream& os)
    {
        ...
        for (auto& p : mS->reuse) {
            ...
            os << std::setw(32) << db.get_attribute(p.first).name() << std::setw(12) << p.second.nodes << std::setw(12)
               << nelem;
...
public:
    ...
    void process_node(CaliperMetadataAccessInterface& db, const Node* node)
    {
        ...
        mS->size_nodes += 3 * 8 + (type == CALI_TYPE_USR || type == CALI_TYPE_STRING ? node->data().size() : 8);
    }

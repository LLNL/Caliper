...
struct ArgRecorder<N, Arg, Args...> {
    ...
    static auto record(const char* name, Arg arg, Args... args) -> decltype(std::tuple_cat(
        ...
        return std::tuple_cat(NextRecorder::record(name, args...), std::forward_as_tuple(std::move(func_annot)));
    }
};
...
struct ArgWrappedFunction {
    ...
    auto operator()(Args... args) ->
        typename std::enable_if<!std::is_same<typename std::result_of<LB(Args...)>::type, void>::value,
...
struct RecordedFunction {
    ...
    auto operator()(Args... args) ->
        ...
        recordAll(args...);
        recording_phase.end();
        cali::Annotation::Guard func_annot(cali::Annotation("debugged_function").begin(name));
        auto                    return_value = body(args...);
        //cali::Annotation return_value_annot("return");
        //return_value_annot.set(return_value);
        //return_value_annot.end();

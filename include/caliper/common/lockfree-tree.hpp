...
private:
    ...
    static LockfreeIntrusiveTree<T>::Node& node(T* t, LockfreeIntrusiveTree<T>::Node T::*node) {
        return (t->*node);
    }

    LockfreeIntrusiveTree<T> tree(T* t) const {
        return LockfreeIntrusiveTree<T>(t, m_node);
    }

    LockfreeIntrusiveTree<T>::Node& node(T* t) const {
        return node(t, m_node);
    }
...
public:
    ...
    T* parent() const {
        return node(m_me).parent;
    }

    T* first_child() const {
        return node(m_me).head.load(std::memory_order_relaxed);
    }

    T* next_sibling() const {
        return node(m_me).next;
    }

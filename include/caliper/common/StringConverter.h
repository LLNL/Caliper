...
public:
    ...
    StringConverter() : m_str() {}

    StringConverter(const std::string& str) : m_str(str) {}
    ...
    std::vector<std::string> to_stringlist(const char* separators = ",", bool* okptr = nullptr) const;

    std::vector<StringConverter> rec_list(bool* okptr = nullptr) const;

    std::map<std::string, StringConverter> rec_dict(bool* okptr = nullptr) const;
};

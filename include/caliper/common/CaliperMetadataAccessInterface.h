...
public:
    ...
    virtual std::vector<Attribute> get_all_attributes() const = 0;

    /// \brief Return all attributes that have a metadata entry \a meta, of any value
    std::vector<Attribute> find_attributes_with(const Attribute& meta) const;

    /// \brief Return all attributes with the given property flag(s)
    std::vector<Attribute> find_attributes_with_prop(int prop) const;

    // --- modifying operations

    virtual Attribute create_attribute(const std::string& name,
                                       cali_attr_type     type,
    ...
    virtual Node* make_tree_entry(std::size_t n, const Node* nodelist[], Node* parent = 0) = 0;

    // --- globals

    /// \brief Return global entries (entries with the \a CALI_ATTR_GLOBAL flag set)
    virtual std::vector<Entry> get_globals() = 0;
};
